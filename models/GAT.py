    ...
    def __init__(self, args):
        ...
        if self.num_layers == 1:
            self.layers_GCN.append(
                self.gat_layer(self.num_feats, self.num_classes, heads=1, concat=True, dropout=self.dropout,
                               bias=False, cached=self.cached))
        elif self.num_layers == 2:
            self.layers_GCN.append(
                self.gat_hidden_layer(self.num_feats, self.dim_hidden, heads=1, concat=True, dropout=self.dropout,
                                      bias=False, cached=self.cached))
            self.layers_GCN.append(
                self.gat_layer(self.dim_hidden, self.num_classes, heads=1, concat=True, dropout=self.dropout,
                               bias=False, cached=self.cached))
        else:
            self.layers_GCN.append(
                self.gat_hidden_layer(self.num_feats, self.dim_hidden, heads=1, concat=True, dropout=self.dropout,
            ...
            for _ in range(self.num_layers - 2):
                self.layers_GCN.append(
                    self.gat_hidden_layer(self.dim_hidden, self.dim_hidden, heads=1, concat=True, dropout=self.dropout,
                                          bias=False, cached=self.cached))
            self.layers_GCN.append(
                self.gat_layer(self.dim_hidden, self.num_classes, heads=1, concat=True, dropout=self.dropout,
                               bias=False, cached=self.cached))
